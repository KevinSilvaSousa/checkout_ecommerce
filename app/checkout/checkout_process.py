...
async def checkout_process(checkout_request: CheckoutRequest, db: AsyncSession = Depends(get_db), payment_client: PaymentClient = Depends(get_payment_client), inventory_client: InventoryClient = Depends(get_inventory_client), order_client: OrderClient = Depends(get_order_client)):
    checkout = Checkout(
    ...
    order_response = await order_response.create(
        checkout_id=checkout.id,
        customer_email = checkout_request.customer_email,
        shipping_address = checkout_request.shipping_adress,
